...
숫자: 시뮬레이션결과.json(⑤), 사후검증_결과.json·순차도입_검정력.json(⑥), 전국진단.json(⑦), 교수브리핑 수치.json
...
HEAD = [
    ...
    ('타분야 데이터\n(선택)', '코레일 안동역 시간대별 승하차(2024.1~2026.8), 한국철도공사 8대 도시(안동) 가명결합 분석(2022.4~6), '
     ...
     '관광지식정보시스템 주요관광지점 입장객, 안동시 버스정보시스템 시간표, 팀 현장조사'),
    ...
    ('계량성과', '[9/26 현장조사 후 입력] 원도심 식당 섭외 동의 ○곳/○곳 · 안내카드 ○장 배포 중 QR 접속 ○건 · '
...
S3_TABLE = [
    ('1단계 식음 → 체험', '원도심 관광 식당(찜닭골목·문화의거리 83곳)에서 영수증 QR 인증 → 체험권(낮: 원도심 인근 체험)', '방문 1위 동에 혜택 0곳. 주민증 앱만으로는 이용률 0.06%'),
